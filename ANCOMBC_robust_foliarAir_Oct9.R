...
# — foliar: 580 differentially abundant ASVs
# 
# ITS: 
#   — biaerosol: 215 differentially abundant ASVs
# — foliar: 245 differentially abundant ASVs
...
# If I did 0.01% it's roughly the same as 200 times at least 
200/1577986*100 # = 0.01267438%
200/1562809*100 # = 0.01279747%
...
I6S_ANCOMall_.01pct_df <- I6S_BAFS_ancom_sigASVtab_t_robust[c(airASVs_8occ.01pct_index_robust, foliarASVs_6occ.01pct_index_robust),]
setdiff(I6S_ANCOMall_.01pct_df$ASV_name, c(bioaerosolI6S_0.01pct_ASVsNames, foliarI6S_0.01pct_ASVsNames)) #these are the same!
# View(I6S_ANCOMall_.01pct_df)
colnames(I6S_ANCOMall_.01pct_df)
# A few checks!
# all look good!
sort(I6S_ANCOMall_.01pct_df$pctBioaerosol[which(I6S_ANCOMall_.01pct_df$ANCOMcat == "bioaerosol")]) #minimum looks good!
sort(I6S_ANCOMall_.01pct_df$pctFoliar[which(I6S_ANCOMall_.01pct_df$ANCOMcat == "foliar surface")]) #minimum looks good!

# Unique families and orders
table(I6S_ANCOMall_.01pct_df$ANCOMcat, I6S_ANCOMall_.01pct_df$Order)
...
I6S_ANCOMall_.01pct_df_long <- I6S_ANCOMall_.01pct_df %>% 
...
head(I6S_ANCOMall_.01pct_df_long)
I6S_nANCOMcatSamp <- I6S_ANCOMall_.01pct_df_long %>% 
...
sum(I6S_ANCOMall_.01pct_df_long$ASV_abundance[intersect(which(I6S_ANCOMall_.01pct_df_long$sampleName == "air_16S_89"), which(I6S_ANCOMall_.01pct_df_long$ANCOMcat == "bioaerosol"))]) == 
...
# saveRDS(I6S_propANCOM_filt_Reads_plot, file = "~/Desktop/CU_Research/SRS_Aeromicrobiome/rObjectsSaved/MS_figures/I6S_propANCOM_filt_Reads_plot_10-9-25.rds")
...
head(readNumberI6SairFoliar_df) #made above, is total number of reads per sample
I6S_ANCOMall_.01pct_df_long_2 <- left_join(I6S_ANCOMall_.01pct_df_long, readNumberI6SairFoliar_df, by = "sampleName")
head(I6S_ANCOMall_.01pct_df_long_2)
# ii. Add a column to get proportion of each ASV in each sample
# Add in prop and sample Type
I6S_ANCOMall_.01pct_df_long_2 <- I6S_ANCOMall_.01pct_df_long_2 %>% 
...
# I6S_ANCOMASVs_list <- I6S_ANCOMall_.01pct_df_long_2 %>% 
#   group_by(ASV_name) %>% 
...
# Make dataframe smaller
colsToKeepANCOM_robust <- c("taxon", "lfc_sampleTypephyllosphere", "W_sampleTypephyllosphere", "q_sampleTypephyllosphere", "diff_sampleTypephyllosphere", "Phylum", "Class",
                     "Order","Family","Genus","Species")
...
foliarITS_0.01pct_ASVsNames <- ITS_BAFS_ancom_sigASVtab_t_robust$ASV_name[ITS_foliarASVs_6occ.01pct_index_robust]
# View(ITS_BAFS_ancom_sigASVtab_t_robust[ITS_foliarASVs_6occ.01pct_index_robust,])

# View(ITS_BAFS_ancom_sigASVtab_t_robust)

###### PLOT AND EXPLORE NEW RESULTS! #####
# 1. Make a new dataframe that just has these new, differentially abundant taxa by concatenating indices from above
ITS_ANCOMall_.01pct_df <- ITS_BAFS_ancom_sigASVtab_t_robust[c(ITSairASVs_11occ.01pct_index_robust, ITS_foliarASVs_6occ.01pct_index_robust),]
setdiff(ITS_ANCOMall_.01pct_df$ASV_name, c(bioaerosolITS_0.01pct_ASVsNames, foliarITS_0.01pct_ASVsNames)) #these are the same!
# View(ITS_ANCOMall_.01pct_df)
colnames(ITS_ANCOMall_.01pct_df)
# A few checks!
# all look good!
sort(unique(ITS_ANCOMall_.01pct_df$pctBioaerosol[which(ITS_ANCOMall_.01pct_df$ANCOMcat == "bioaerosol")])) #minimum looks good!
sort(unique(ITS_ANCOMall_.01pct_df$pctFoliar[which(ITS_ANCOMall_.01pct_df$ANCOMcat == "foliar surface")])) #minimum looks good!
...
colnames(ITS_ANCOMall_.01pct_df)
# i. Get each ASV's abundance in each sample
ITS_ANCOMall_.01pct_df_long <- ITS_ANCOMall_.01pct_df %>% 
  pivot_longer(cols = phyllo_ITS_1:air_ITS_99, names_to = "sampleName", values_to = "ASV_abundance")
# ii. Grouping by sampName and ANCOMtype, get sum of ANCOM type in each sample
head(ITS_ANCOMall_.01pct_df_long)
ITS_nANCOMcatSamp <- ITS_ANCOMall_.01pct_df_long %>% 
  select(c(ASV_name, ANCOMcat, sampleName, ASV_abundance)) %>% 
  group_by(ANCOMcat, sampleName) %>% 
  summarize(nANCOMsamp = sum(ASV_abundance))
...
sum(ITS_ANCOMall_.01pct_df_long$ASV_abundance[intersect(which(ITS_ANCOMall_.01pct_df_long$sampleName == "air_ITS_112"), which(ITS_ANCOMall_.01pct_df_long$ANCOMcat == "bioaerosol"))]) == 
...
# saveRDS(ITS_propANCOM_filt_Reads_plot, file = "~/Desktop/CU_Research/SRS_Aeromicrobiome/rObjectsSaved/MS_figures/ITS_propANCOM_filt_Reads_plot_10-9-25.rds")

head(ITS_ANCOMall_.01pct_df_long)
# i. Add in number of reads per sample
head(readNumberITSairFoliar_df) #made above, is total number of reads per sample
ITS_ANCOMall_.01pct_df_long_2 <- left_join(ITS_ANCOMall_.01pct_df_long, readNumberITSairFoliar_df, by = "sampleName")
head(ITS_ANCOMall_.01pct_df_long_2)
# ii. Add a column to get proportion of each ASV in each sample
# Add in prop and sample Type
ITS_ANCOMall_.01pct_df_long_2 <- ITS_ANCOMall_.01pct_df_long_2 %>% 
  mutate(propASVPerSamp = ASV_abundance/TotalReads) %>% 
  mutate(
    sampleType = case_when(
      str_detect(sampleName, "air") ~ "air",
...
# ITS_ANCOMASVs_ANCOMcatlist <- ITS_ANCOMall_.01pct_df_long_2 %>%
#   group_by(ANCOMcat) %>%
#   group_split(.keep = TRUE)
...
#     { setNames(.$data, .$ASV_name) }
# })
# names(ITS_FS_orders_nestedASVsList)[5] #e.g. CapnodialesFS_ANCOM
# names(ITS_FS_orders_nestedASVsList[[5]]) #shows ASVs within AgaricalesBA_ANCOM
# length(ITS_FS_orders_nestedASVsList[[5]]) #18
# 
...
# names(ITS_BA_orders_nestedASVsList)[15]
# Polyporales_BA_PlotsList <- vector(mode = "list", length = length(ITS_BA_orders_nestedASVsList[[15]]))
# for (i in seq_along(ITS_BA_orders_nestedASVsList[[15]])) {
#   df <- ITS_BA_orders_nestedASVsList[[15]][[i]]
#   ttl <- paste0("ITS_BA_",names(ITS_BA_orders_nestedASVsList[[15]])[i], "_",
#                 paste(unique(df$Family), collapse = ", "))
#   
...
# length(Polyporales_BA_PlotsList) #246!!!
# # Capnodiales F.S. indicators
# Capnodiales_FS_PlotsList <- vector(mode = "list", length = length(ITS_FS_orders_nestedASVsList[[5]]))
# for (i in seq_along(ITS_FS_orders_nestedASVsList[[5]])) {
#   df <- ITS_FS_orders_nestedASVsList[[5]][[i]]
#   ttl <- paste0("ITS_FS_",names(ITS_FS_orders_nestedASVsList[[5]])[i], "_",
#                 paste(unique(df$Family), collapse = ", "))
#   
...
#   )
# }
# length(Capnodiales_FS_PlotsList) #184!!!