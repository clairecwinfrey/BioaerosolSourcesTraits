...
# (Benefits versus Class: shows more of the diff abund. ASVs and matches heat maps)
# 1. Get number of ASVs in each Class
numberClassesANCOM_ITS <- ITS_ANCOMall_.05pct_df %>% 
  group_by(ANCOMcat, Class) %>% 
  summarise(ASVsInClassByHab = n_distinct(ASV_name), .groups = "drop") %>%  #.groups = "drop" no more grouping
  complete(ANCOMcat, Class, fill = list(ASVsInClassByHab = 0)) #add in zero if not found in that ANCOM cat
# View(numberClassesANCOM_ITS)
# Check calculations
table(ITS_ANCOMall_.05pct_df$Class, ITS_ANCOMall_.05pct_df$ANCOMcat) #these match the calculations above!
unique(ITS_ANCOMall_.05pct_df$Class) #only 17, which maybe could fit except some are very ASV-rich so bubbles likely too big
sum(numberClassesANCOM_ITS$ASVsInClassByHab) #220
sum(numberClassesANCOM_ITS$ASVsInClassByHab) == length(unique(ITS_ANCOMall_.05pct_df$ASV_name)) #220

allnumberClassesANCOM_ITS <- numberClassesANCOM_ITS[-which(numberClassesANCOM_ITS$Class == "NA"),] #drop NA classes
dim(allnumberClassesANCOM_ITS)
sum(allnumberClassesANCOM_ITS$ASVsInClassByHab) #shows 215/220
sum(allnumberClassesANCOM_ITS$ASVsInClassByHab[which(allnumberClassesANCOM_ITS$ANCOMcat == "foliar surface")]) #123/___ foliar surface ASVs
sum(allnumberClassesANCOM_ITS$ASVsInClassByHab[which(allnumberClassesANCOM_ITS$ANCOMcat == "bioaerosol")]) #92/___ biaoerosol ASVs

# Add phylum info to this plot by merging with subset of ITS_ANCOMall_.05pct_df that has phylum info! DO distinct because otherwise subset is v long and get many-to-many relationship
classPhylaITS <- distinct(ITS_ANCOMall_.05pct_df[,colnames(ITS_ANCOMall_.05pct_df) %in% c("Class", "Phylum")])
allnumberClassesANCOM_ITS <- left_join(allnumberClassesANCOM_ITS, classPhylaITS, by = "Class")
# View(allnumberClassesANCOM_ITS) # looks good!

# Re-order classes (making it a factor) based on alphabetical in phylum than in Class
allnumberClassesANCOM_ITS <- allnumberClassesANCOM_ITS %>%
  arrange(desc(Phylum), desc(Class)) %>%  #alphabetic by Phylum, then Class, descending so that it goes up y-axis
  mutate(Class = factor(Class, levels = unique(Class))) 
# View(allnumberClassesANCOM_ITS) #Looks good!
allnumberClassesANCOM_ITS$Class
unique(allnumberClassesANCOM_ITS$Class)
levels(allnumberClassesANCOM_ITS$Class)

# Now remove those with 0 so that there are no dots for zero (had to have these zeros first to avoid arranging by ANCOMcat)
allnumberClassesANCOM_ITS_no0s <- allnumberClassesANCOM_ITS[-which(allnumberClassesANCOM_ITS$ASVsInClassByHab == 0),]
# View(allnumberClassesANCOM_ITS_no0s)
levels(allnumberClassesANCOM_ITS_no0s$Class) #great kept levels!
# Make this numeric to squeeze left and right dots closer together
allnumberClassesANCOM_ITS_no0s$xpos <- as.numeric(factor(allnumberClassesANCOM_ITS_no0s$ANCOMcat))

# Make the plot!
colnames(allnumberClassesANCOM_ITS_no0s)
ITS_allClassANCOM_bubPlot <- ggplot(allnumberClassesANCOM_ITS_no0s,
                                    aes(x = xpos, y = Class, size = ASVsInClassByHab, fill = ANCOMcat)) +
  geom_point(shape = 21, color = "black", stroke = 0.3) +
  scale_size(range = c(1, 6)) +   
  facet_grid(Class ~ ., scales = "free_y", space = "free_y", switch = "y", as.table = FALSE) + #as.table = FALSE orders by factor
  scale_x_continuous(
    breaks = 1:2,  # positions of the categories
    labels = levels(factor(allnumberClassesANCOM_ITS$ANCOMcat)),
    expand = expansion(mult = c(0.0, 0.0)),
    limits = c(0.2, 3)  # pull the two categories closer together
  ) +
  theme_bw() +
