...
allITSr_top_orders
# ########################################################
# # INVESTIGATING HOW AVERAGE DISSIMILARITY BETWEEN FOREST AND SAVANNA DIFFER
# # i) due to wind!
# ########################################################
# 
# # Load wind data
# windData <- read.csv("~/Desktop/CU_Research/SRS_Aeromicrobiome/BioinformaticsAndMetadata/windDataLoggerApril23_2023.csv")
# # View(windData)
# # Hellinger transformation then Bray-Curtis dissimilarities for the air samples only (no controls) data. 
# 
# ##### FIRST, FOR ALL SAMPLES #####
# # Get it out of phyloseq and transpose (i.e. rows = samples, cols = ASVs) to make it vegan-compatible.
# airITS_noSingDoubsASVs <- t(as.data.frame(as.matrix(otu_table(airITS_noSingDoubs.ps))))
# # View(airITS_noSingDoubsASVs) #samples are rows and ASVs are columns
# 
# # Hellinger transformation
# airITS_noSingDoubsASVs_h <- decostand(airITS_noSingDoubsASVs, method = "hellinger")
# # View(airITS_noSingDoubsASVs_h)
# 
# # Get Bray-Curtis dissimilarity
# airITSr_BCdist <- vegdist(airITS_noSingDoubsASVs_h, method= "bray")
# 
# 
# ##### NOW FOR EACH DIFFERENT SAMPLING DATE #####
# # 1) how many samples left remaining in each?
# sampsByDate <- as.data.frame(matrix(ncol=2, nrow=length(daysOutByEU)))
# sampsByDate$V1 <- daysOutByEU # NOTE:  daysOutByEU in NOT in complete chronological order;
# # i.e., it has the chronological order of the 4 EU 52 dates, then the four from EU 8, etc.
# # Specifically, this for loop first subsets data by day set out (again, EU by EU) to make 16 
# # separate phyloseq objects, and it then gets the number of samples that remained in analysis
# # (after dropping bad samples, for whatever reason) for each of these
# for (j in 1:length(ITS_byDate_ps_list)){
#   sampsByDate[j,2] <- ncol(otu_table(ITS_byDate_ps_list[[j]]))
# }
# names(ITS_byDate_ps_list) #see these are by EU date
# # Get rid of samples for which there are fewer than 5 comparisons
# which(sampsByDate$V2 >= 5) #all days have at least 5 comparisons
# 
# # FIRST FOR LOOP: Get hellinger-ized BC dists and the forest or savanna categories
# # Pre-allocate vectors 
# ITS_ASVtabByDayList <- vector("list", length=length(daysOutByEU)) # pre-allocate list to hold all the transposed ASV tables
# ITS_hellingerASVsList <- vector("list", length=length(daysOutByEU)) # pre-allocate list to hold all the Hellinger-transformed ASV tabs
# ITS_BC_h_distList <- vector("list", length=length(daysOutByEU)) # pre-allocate list to hold all the distances
# ITS_dataForLoopList <- vector("list", length=length(daysOutByEU)) # pre-allocate list to hold savanna/forest info
# # THE LOOP HERSELF
# for (k in 1:length(ITS_byDate_ps_list)){ # ITS_byDate_ps_list has one day per list phyloseq object. AGAIN:  daysOutByEU in NOT in complete chronological order;
#   # i.e., it has the chronological order of the 4 EU 52 dates, then the four from EU 8, etc.
#   ITS_ASVtabByDayList[[k]] <- t(as.data.frame(as.matrix(otu_table(ITS_byDate_ps_list[[k]])))) #transpose and get each ASV tab out of phyloseq
#   names(ITS_ASVtabByDayList)[[k]] <- names(ITS_byDate_ps_list)[[k]]
#   #get Hellinger transformed of each ASV table (transposed). Order is same as ITS_byDate_ps_list, which is what is wanted!
#   ITS_hellingerASVsList[[k]] <- decostand(ITS_ASVtabByDayList[[k]], method = "hellinger") 
#   names(ITS_hellingerASVsList)[[k]] <- names(ITS_ASVtabByDayList)[[k]]
#   ITS_BC_h_distList[[k]] <- as.matrix(vegdist(ITS_hellingerASVsList[[k]], method= "bray")) #this makes ITS_BC_h_distList in the correct order!
#   names(ITS_BC_h_distList)[[k]] <- names(ITS_hellingerASVsList)[[k]]
#   diag(ITS_BC_h_distList[[k]]) <- NA
#   ITS_dataForLoopList[[k]] <- cbind(rownames(sample_data(ITS_byDate_ps_list[[k]])), sample_data(ITS_byDate_ps_list[[k]])$HabitatAir)
#   names(ITS_dataForLoopList)[[k]] <- names(ITS_byDate_ps_list)[[k]]
#   for (j in 1:length(rownames(sample_data(ITS_byDate_ps_list[[k]])))){
#     if (rownames(ITS_BC_h_distList[[k]])[j] == ITS_dataForLoopList[[k]][j,1]){
#       rownames(ITS_BC_h_distList[[k]])[j] <- ITS_dataForLoopList[[k]][j,2]
#     }
#     if (colnames(ITS_BC_h_distList[[k]])[j] == ITS_dataForLoopList[[k]][j,1]){
#       colnames(ITS_BC_h_distList[[k]])[j] <- ITS_dataForLoopList[[k]][j,2]
#     }
#   }
# }
# 
# names(ITS_byDate_ps_list) #this shows that still ordered by EU and then dates that that EU was sampled
# 
# # SECOND FOR LOOP: This for loop creates a list of 16 dataframes, where each row is the row and column of the 
# # value that we want from each date set out
# ITS_savColsList <- vector("list", 16) # pre-allocate list
# ITS_forestRowsList <- vector("list", 16) # pre-allocate list
# ITS_combosList <- vector("list", 16) # pre-allocate list
# names(ITS_combosList) <- daysOutByEU #
# 
# #### STOPPED HERE SEPT. 12 2023 #####
# 
# for (i in 1:length(ITS_BC_h_distList)){
#   ITS_savColsList[[i]] <- which(colnames(ITS_BC_h_distList[[i]])== "savanna")
#   ITS_forestRowsList[[i]] <- which(rownames(ITS_BC_h_distList[[i]])== "forest")
#   ITS_combosList[[i]] <- expand.grid(ITS_savColsList[[i]],ITS_forestRowsList[[i]])
#   ITS_combosList[[i]]$Combined <-paste(ITS_combosList[[i]]$Var1,ITS_combosList[[i]]$Var2,sep=",") #this is now the rows, columns we want!
# }
# 
# # NESTED FOR LOOP THAT PULLS OUT RELEVANT ITS_BCs FOR EACH DAY AND THEN AVERAGES THEM
# # Nested for loop to pull out all of the relevant ITS_BCs for each of the days
# ITS_BCs <- vector("list", 16) #preallocate a vector to store all of the ITS_BCs for the various days
# ITS_BCsAvg <- vector(length=16) #preallocate a vector with the same number of spots as number of days
# # first for loop to go over each day
# for (i in 1:length(ITS_BC_h_distList)){
#   # Second, nested for loop pulls out the specific desired BC distances within the given day
#   for (j in 1:unname(sapply(ITS_combosList, nrow)[i])){ #this loops over all of the rows in ITS_combosList[[i]]
#     # Line below takes the ith iteration of ITS_BC_h_distList, and then gets the value in the jth row of Var1 and the value in the jth row of var 2.
#     # This var1 and var2 values are the index [var1, var2] that are pulled out of ITS_BC_h_distList[[i]] and which are assigned to
#     # the jth row in ITS_BCs
#     ITS_BCs[[i]][j] <- ITS_BC_h_distList[[i]][(ITS_combosList[[i]]$Var1[j]), (ITS_combosList[[i]]$Var2[j])] 
#     ITS_BCsAvg[i] <- mean(ITS_BCs[[i]]) #this gets the mean for the ITS_BCs just pulled out.
#   } 
# }
# 
# # TEST NESTED FOR LOOP BY TRYING WITH FIRST ONE
# ITS_BCs1 <- c() #make a vector with as many slots as rows in ITS_combosList
# ITS_BCsAvg1 <- c() #make a vector with the same number of spots as number of days
# # This nested for loop pulls out the specific desired BC distances within the given day
# for (j in 1:unname(sapply(ITS_combosList, nrow)[1])){ #this loops over all of the rows in ITS_combosList[1]
#   ITS_BCs1[j] <- ITS_BC_h_distList[[1]][(ITS_combosList[[1]]$Var1[j]), (ITS_combosList[[1]]$Var2[j])]
#   ITS_BCsAvg1 <- mean(ITS_BCs1) #this gets the mean for the ITS_BCs just pulled out.
# } 
# 
# ITS_BCs[[1]] == ITS_BCs1 #this shows that this part of the nested for loop worked
# ITS_BCsAvg[1] == ITS_BCsAvg1 #this works!
# 
# # TEST A FEW TO MAKE SURE IT'S WORKING 
# ITS_BCsAvg[2]
# ITS_byDate_ps_list[[2]] #this date has 7 samples
# airITS_noSingDoubsASVs_2 <- t(as.data.frame(as.matrix(otu_table(ITS_byDate_ps_list[[2]]))))
# # View(airITS_noSingDoubsASVs_2)
# # HELLINGER TRANSFORMATION
# airITS_noSingDoubsASVs_h_2 <- decostand(airITS_noSingDoubsASVs_2, method = "hellinger")
# # View(airITS_noSingDoubsASVs_h_2)
# # Get Bray-Curtis dissimilarity
# airITSr_BCdist_h2 <- vegdist(airITS_noSingDoubsASVs_h_2, method= "bray")
# airITSr_BCdist_h2
# 
# ITS_BCsAvg[5]
# ITS_byDate_ps_list[[5]] #this date has 7 samples
# airITS_noSingDoubsASVs_5 <- t(as.data.frame(as.matrix(otu_table(ITS_byDate_ps_list[[5]]))))
# # View(airITS_noSingDoubsASVs_5)
# # HELLINGER TRANSFORMATION
# airITS_noSingDoubsASVs_h_5 <- decostand(airITS_noSingDoubsASVs_5, method = "hellinger")
# # View(airITS_noSingDoubsASVs_h_5)
# # Get Bray-Curtis dissimilarity
# airITSr_BCdist_h_5 <- vegdist(airITS_noSingDoubsASVs_h_5, method= "bray")
# airITSr_BCdist_h_5
# 
# ITS_BCsAvg[8]
# ITS_byDate_ps_list[[8]] #this date has 7 samples
# airITS_noSingDoubsASVs_8 <- t(as.data.frame(as.matrix(otu_table(ITS_byDate_ps_list[[8]]))))
# # View(airITS_noSingDoubsASVs_8)
# # HELLINGER TRANSFORMATION
# airITS_noSingDoubsASVs_h_8 <- decostand(airITS_noSingDoubsASVs_8, method = "hellinger")
# # View(airITS_noSingDoubsASVs_h_8)
# # Get Bray-Curtis dissimilarity
# airITSr_BCdist_h_8 <- vegdist(airITS_noSingDoubsASVs_h_8, method= "bray")
# airITSr_BCdist_h_8
# 
# # GET WIND DATA INTO SHAPE
# # Pull out the (non-repeating) combinations of average speed, EU, and sampling day from wind data
# # "atualDaysOrder is the actual days that it was sampled in chronological order, i.e., NOT
# # EU by EU (unlike daysOutByEU)
# # actualDaysOrder is the actual order by the calendar.
# avgWindByDay <- windData %>% tidyr::expand(nesting(avgSpeedSampPeriod, EU, actualDaysOrder))
# avgWindByDay$actualDaysOrder
# 
# # Re-arrange so that avgWindByDay is arranged by actualDaysOrder (in ascending order)
# avgWindByDay <- avgWindByDay %>% arrange(actualDaysOrder)
# 
# # The ITS_BCsAvg, below, are not in the same order as are the days in avgWindByDay (which are stupidly and unhelpfully
# # automatically ordered by avgSpeedSampPeriod).
# 
# daysOutByEU #Created higher up in script, but take a look here again)
# # ITS_BCsAvg is currently in the same order as daysOutByEU; below will re-order it
# 
# # Read in this CSV, which pairs dayOut (same as actual day order), with daysOutByEUorder_inBCSAVG (how daysOutByEU and ITS_BCsAvg are)
# dayOutToDateByEU <- read.csv("~/Desktop/CU_Research/SRS_Aeromicrobiome/BioinformaticsAndMetadata/dayOutToDateByEU.csv")
# colnames(dayOutToDateByEU)[1] <- "DateSetOut" #re-name this for ease of merging way down below
# sort(unique(dayOutToDateByEU$daysOut))
# colnames(avgWindByDay)[3] <- "daysOut" #rename this for easier merging
# sort(unique(avgWindByDay$daysOut))
# 
# # Merge this so that there is now a "key" connecting daysOutByEU and the actual order (and to have the wind data)
# allTheWaysOfDays <- merge(dayOutToDateByEU, avgWindByDay, by="daysOut", all.x = TRUE)
# # Re-arrange so that it's in the same order as ITS_BCsAvg
# allTheWaysOfDaysOrdered <- allTheWaysOfDays %>%  arrange(daysOutByEUorder_inBCSAVG) #looks good!
# 
# # Now, one more cbind so that the ITS_BCsAvg is included!
# windDat_AvgBCs <- cbind(allTheWaysOfDaysOrdered, ITS_BCsAvg)
# colnames(windDat_AvgBCs)
# 
# # TEST AVERAGE WIND SPEED AND AVERAGE DISTANCE BETWEEN FOREST AND SAVANNA
# cor.test(windDat_AvgBCs$ITS_BCsAvg, windDat_AvgBCs$avgSpeedSampPeriod) #0.1912289 #this looks the same as it did with the other method, confirming it worked
# lmITS_BCsAvgWind <- lm(ITS_BCsAvg ~ avgSpeedSampPeriod, data=windDat_AvgBCs)
# summary(lmITS_BCsAvgWind)
# 
# # Coefficients:
# #   Estimate Std. Error t value Pr(>|t|)    
# # (Intercept)         0.38855    0.04251   9.141 2.81e-07 ***
# #   avgSpeedSampPeriod  0.03456    0.04741   0.729    0.478    
# # ---
# #   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# # 
# # Residual standard error: 0.06584 on 14 degrees of freedom
# # Multiple R-squared:  0.03657,	Adjusted R-squared:  -0.03225 
# # F-statistic: 0.5314 on 1 and 14 DF,  p-value: 0.478
# # #  
# 
# # PLOT IT 
# # Plot to see what relationship looks like: This shows that as the average wind speed goes up, so does
# # the distance among forest and savanna samples on a given day
# airITS_BCsWindByDatePlot <- ggplot(windDat_AvgBCs, aes(x=avgSpeedSampPeriod, y=ITS_BCsAvg)) +
#   geom_point(color="black",size=3.5) +
#   geom_smooth(method=lm , color="black", fill="#69b3a2", se=TRUE) + 
#   # scale_colour_manual(values=c(`52`="#0000FF",`53S`="#FF0000",`54S`="#00FF00",`8`="#FF00B6"))+ #THIS LINE ISN'T WORKING TO CHANGE COLOR
#   scale_x_continuous(name= "average wind speed (kph)", breaks= c(0, 0.25, 0.5, 0.75, 1.0, 1.25), labels = c(0, 0.25, 0.5, 0.75, 1.0, 1.25)) +
#   #scale_y_continuous(name="average B-C dist between savanna and forest", breaks= c(0.15, 0.175, 0.20, 0.225, 0.25, 0.275), labels= c(0.15, 0.175, 0.20, 0.225, 0.25, 0.275)) +
#   theme_bw() + ggtitle("Relationship between forest/savanna dist and wind speed") +
#   #labs(title="Relationship between Dispersion and Wind Speed",x="average wind speed (kph)", y = "dispersion") +
#   theme(legend.text = element_text(colour="black", size = 12))  + theme(legend.title = element_blank()) +
#   theme(axis.text.x=element_text(size=14)) + theme(axis.text.y =element_text(size=14)) + theme(axis.title = element_text(size=14)) +
#   theme(plot.title=element_text(size=16)) + scale_color_manual(values = colsForEUs)
# 
# # quartz()
# airITS_BCsWindByDatePlot
# 
# # Stopped here Monday, Sept. 25, 2023
# 
# ################################
# # PERFORM GLOBAL DB-RDA
# ###############################
# # FIRST GET META DATA FIGURED OUT
# metDat <- as.data.frame(as.matrix(sample_data(airITS_noSingDoubs.ps)))
# str(metDat)
# metDat$TransectMeter
# metDat$daysOut <- as.numeric(metDat$daysOut)
# metDat$MaxPumpT <- as.numeric(metDat$MaxPumpT)
# metDat$AvgPumpT <- as.numeric(metDat$AvgPumpT)
# str(metDat)
# # Want to add in average wind data to this data frame. The two variables that are the same in metData and avgWindByDay
# # are avgWindByDay$actualDaysOrder and metDat$daysOut
# 
# # Merge avgWindByDay and metDat to set up for dbRDA
# newMetaDat <- merge(metDat, allTheWaysOfDaysOrdered, by= c("DateSetOut", "EU"), all.x= TRUE)
# # View(newMetaDat)
# rownames(newMetaDat) <- rownames(metDat)
# # View(newMetaDat)
# str(newMetaDat)
# newMetaDat$daysOut.x == newMetaDat$daysOut.y #these are the same so one an be dropped
# newMetaDat$daysOut.x <- NULL #drop it to make df smaller
# # Finally, drop other columns that are unfilled but that will mess up merging with "littleSoilMetaDat" made below
# newMetaDat$meanDens <- NULL
# newMetaDat$Percent_Vegetation_Cover <- NULL
# newMetaDat$AvgVegTran <- NULL
# newMetaDat$AvgCanTran <- NULL
# newMetaDat$mean_pH <- NULL
# # View(newMetaDat)
# 
# # Bring in canopy cover, pH, and veg cover from soil, based on getting transect mapping
# soilMetaDat <- as.data.frame(as.matrix(sample_data(soilITSr_noControlsTrimmed.ps))) #we'll get it from just soil
# sort(colnames(soilMetaDat))
# #View(soilMetaDat)
# 
# soilMetaDat$Meter <- gsub(soilMetaDat$Meter, pattern=" ", replacement = "") #get rid of weird spaces in meter
# # For soil, the meter labeling was different than it was in the air project; all of the transect meters are
# # shifted down 10 meters in air due to a difference in labeleing So, meter 10 (soil) is meter 0 (air) and meter 90 (soil)
# # is meter 80 (air). Since these are the two that I care about, I'll first filter so that I only keep meters 
# # "90" and "10", and then, I'll change these to 80 and 0, respectively. 
# soilMetaDat_filtered <- soilMetaDat %>% #remove all rows but those at meter 10 or 90
#   filter(Meter == "90" | Meter == "10")
# # View(soilMetaDat_filtered)
# soilMetaDat_filtered$Meter <- gsub(soilMetaDat_filtered$Meter, pattern="10", replacement = "0") #this looks good. Now, "10" is "0"
# soilMetaDat_filtered$Meter <- gsub(soilMetaDat_filtered$Meter, pattern="90", replacement = "80") #this looks good. Now, "90" is "80"
# soilMetaDat_filtered$TransectMeter <- paste(soilMetaDat_filtered$Transect, soilMetaDat_filtered$Meter, sep="") #add a "TransectMeter" for merging below!
# # View(soilMetaDat_filtered)
# 
# # colnames(soilMetaDat_filtered)
# littlesoilMetaDat_filtered <- soilMetaDat_filtered[,c(1, 2, 33,34,35,36,37)] #get EU, TransectMeter, meanDens (i.e. canopy cover), veg cover, AvgVegTran, AvgCanTran, and mean_pH
# head(littlesoilMetaDat_filtered)
# rownames(littlesoilMetaDat_filtered) <- NULL #remove row-names
# intersect(colnames(newMetaDat), colnames(littlesoilMetaDat_filtered))
# intersect(sort(unique(newMetaDat$TransectMeter)), sort(unique(littlesoilMetaDat_filtered$TransectMeter))) #great, they match on TransectMeter!
# intersect(sort(unique(newMetaDat$EU)), sort(unique(littlesoilMetaDat_filtered$EU))) #great, they match on TransectMeter!
# metDat2 <- merge(newMetaDat, littlesoilMetaDat_filtered, by=c("TransectMeter", "EU"), all.x=TRUE)
# row.names(metDat2) <- row.names(newMetaDat)
# #View(metDat2)
# metDat2[which(is.na(metDat2$mean_pH)),]
# colnames(metDat2)
# # Why are there data points missing?
# # The following lines show that this is correct; there is just no soil physicochemical data for these
# littlesoilMetaDat_filtered %>% 
#   filter(TransectMeter == "B80" & EU == "EU_52")
# littlesoilMetaDat_filtered %>% 
#   filter(TransectMeter == "R80" & EU == "EU_8")
# newMetaDat %>% 
#   filter(TransectMeter == "B0" & EU == "EU_52")
# soilMetaDat %>% 
#   filter(Transect == "B" & EU == "EU_52" & Meter == "90")
# soilMetaDat %>% 
#   filter(Transect == "R" & EU == "EU_8" & Meter == "90")
# 
# # For some reason, these ended up getting droppped, but I hand-checked many and saw soil metadat right. Will investigate more later,
# # but for now, enter manually based on data in "SRS_16S_rawPsMeta.xlsx"
# # REPLACE EU 52 B80
# colnames(metDat2)
# rowNumbs_52_B80 <- which(metDat2$TransectMeter == "B80" & metDat2$EU == "EU_52")
# for (j in 1:length(rowNumbs_52_B80)){
#   metDat2[rowNumbs_52_B80[j],c(40:44)] <- c(73.50, 68, 69.25, 60.2500, 4.539885)
# }
# metDat2[rowNumbs_52_B80,c(40:44)]
# 
# # REPLACE EU 8 B80
# colnames(metDat2)
# rowNumbs_8_R80 <- which(metDat2$TransectMeter == "R80" & metDat2$EU == "EU_8")
# for (j in 1:length(rowNumbs_8_R80)){
#   metDat2[rowNumbs_8_R80[j],c(40:44)] <- c(95.75, 2, 25.50, 90.8750, 3.879885)
# }
# metDat2[rowNumbs_8_R80,c(40:44)]
# 
# # Finally, clean up variable categories in metDat2
# str(metDat2)
# metDat2$meanDens <- as.numeric(metDat2$meanDens)
# metDat2$Percent_Vegetation_Cover <- as.numeric(metDat2$Percent_Vegetation_Cover)
# metDat2$AvgVegTran <- as.numeric(metDat2$AvgVegTran)
# metDat2$AvgCanTran <- as.numeric(metDat2$AvgCanTran)
# metDat2$mean_pH <- as.numeric(metDat2$mean_pH)
# 
# # MAKE DB-RDA!!!
# # below from Dec. 4, 2023
# airITSr_savFor_mod <- dbrda(airITSr_BCdist ~ HabitatAir + Condition(daysOutByEUorder_inBCSAVG), data = metDat2)
# set.seed(19)
# airITSr_savFor_mod_Results <- anova.cca(airITSr_savFor_mod, permutations= 9999, by="margin")
# airITSr_savFor_mod_Results 
# # Permutation test for dbrda under reduced model
# # Marginal effects of terms
# # Permutation: free
# # Number of permutations: 9999
# # 
# # Model: dbrda(formula = airITSr_BCdist ~ HabitatAir + Condition(daysOutByEUorder_inBCSAVG), data = metDat2)
# # Df SumOfSqs      F Pr(>F)
# # HabitatAir   1    0.155 0.7541 0.6437
# # Residual   107   21.998   
# 
# # Trying without the conditioning effect
# airITSr_savForNoCond_mod <- dbrda(airITSr_BCdist ~ HabitatAir, data = metDat2)
# set.seed(19)
# airITSr_savForNoCond_mod_Results <- anova.cca(airITSr_savForNoCond_mod, permutations= 9999, by="margin")
# airITSr_savForNoCond_mod_Results  #  Sum of squares: 0.1523, F Pr(>F) 0.1523 0.7385 0.6743
# 
# adonis2(airITSr_BCdist ~ HabitatAir, data= metDat2, permutations = 999, method = "bray")
# 
# ################ MODEL SELECTION ##################
# 
# #1. mMke model with only intercept 
# fung_dbrda.0 <- dbrda(airITSr_BCdist ~1, data = metDat2) 
# 
# #2. Make mod with all possible (relevant variables)- EU, DateSetOut, AvgSpeed, Habitat (same as HabitatAir),
# # average wind speed, canopy cover (for that one), canopy cover (for that transect),  veg cover, veg cover for
# # transect (not mean pH since 
# smolMetaDat2 <- metDat2[,c(2,3, 20, 39:43)]
# smolMetaDat_2 <- metDat2[,c(2,3, 20, 39:41)]
# str(smolMetaDat_2 )
# 
# fung_dbrda.all <- dbrda(airITSr_BCdist ~ ., data = smolMetaDat2) 
# fung_dbrda.all2 <- dbrda(airITSr_BCdist ~ ., data = smolMetaDat_2) 
# fung_dbrda.all3 <- dbrda(airITSr_BCdist ~ ., data = smolMetaDat_2[,c(5,6)]) 
# fung_dbrda.all4 <- dbrda(airITSr_BCdist ~ ., data = smolMetaDat_2[,c(1,3,4,5,6)]) #best maybe!
# fung_dbrda.all2_Results <- anova.cca(fung_dbrda.all2, permutations= 9999,by="margin")
# fung_dbrda.all2_Results
# fung_dbrda.all3_Results <- anova.cca(fung_dbrda.all3, permutations= 9999, by="margin")
# fung_dbrda.all3_Results
# fung_dbrda.all4_Results <- anova.cca(fung_dbrda.all4, permutations= 9999, by="margin")
# fung_dbrda.all4_Results
# 
# #3. double-stopping forward model sel
# set.seed(93)
# fungAllForSel <- ordiR2step(fung_dbrda.0, scope = fung_dbrda.all, permutations = 9999)
# fungAllForSelresults <- fungAllForSel$anova
# fungAllForSelresults 
# 
# set.seed(93)
# fungAllForSel2 <- vegan::ordiR2step(fung_dbrda.0, scope = fung_dbrda.all2, permutations = 9999)
# fungAllForSelresults2 <- fungAllForSel$anova
# fungAllForSelresults 
# 
# vegan::ordiR2step(fung_dbrda.0, scope = fung_dbrda.all3, permutations = 9999)
# fung_dbrda.all3
# 
# set.seed(93)
# fungAllForSel4 <- vegan::ordiR2step(fung_dbrda.0, scope = fung_dbrda.all4, permutations = 9999)
# fungAllForSelresults2 <- fungAllForSel$anova
# fungAllForSelresults 
# 
# # PLOT AN NMDS PLOT THAT REPRESENTS THE VARIABLES FROM THE SIGNIFICANT VARIABLES! 
# # this page helped me make a pretty plot! 
# # https://jkzorz.github.io/2019/06/06/NMDS.html
# # see here for more ways to jazz it up: https://jkzorz.github.io/2020/04/04/NMDS-extras.html
# set.seed(123)
# fungAir_h_nmds <- metaMDS(airITSr_BCdist)
# plot(fungAir_h_nmds) #here the samples are circles (no species data)
# 
# # Extract NMDS scores (x and y coordinates)
# fungAir_h.scores <- as.data.frame(scores(fungAir_h_nmds))
# 
# # Add columns to data frame 
# rownames(smolMetaDat_2) == rownames(fungAir_h.scores) #since this is true, I can cbind them!
# fungAir_h.scores$Sample <- rownames(fungAir_h.scores) #add in sample column
# fungAir_h.scores$Habitat <- smolMetaDat_2$Habitat
# fungAir_h.scores$meanDens <- smolMetaDat_2$meanDens
# rownames(fungAir_h.scores) <- NULL # drop rownames so that it's tidy!
# head(fungAir_h.scores)
# 
# # Get env fit data:
# fungEnvFit <- envfit(fungAir_h_nmds, smolMetaDat_2[,c(1,3,4,5,6)], permutations = 999, na.rm = TRUE)
# # Extract info from continuous and categorical separately
# fungEnCoordCont <- as.data.frame(scores(fungEnvFit, "vectors")) * ordiArrowMul(fungEnvFit)
# fungEnCoordCat <- as.data.frame(scores(fungEnvFit, "factors")) * ordiArrowMul(fungEnvFit)
# 
# 
# fungHabCanNMDS <- ggplot(fungAir_h.scores, aes(x = NMDS1, y = NMDS2)) + 
#   geom_point(size = 5, aes(color = Habitat)) + theme_bw() +
...
# # Build the ggplot:
# fungHabCanNMDS <- ggplot(fungAir_h.scores, aes(x = NMDS1, y = NMDS2)) + 
#   geom_point(size = 5, aes(color = Habitat)) + 
#   geom_segment(aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
#                data = fungEnCoordCont, size =1, alpha = 0.5, colour = "grey20") +
#   geom_point(data = fungEnCoordCat, aes(x = NMDS1, y = NMDS2), 
#              shape = "diamond", size = 5, alpha = 0.6, colour = "navy") +
#   geom_text(data = fungEnCoordCat, aes(x = NMDS1, y = NMDS2+0.04), 
#             label = c("EU_52", "EU_53S", "EU_54S", "EU_8", "forest", "savanna"), colour = "navy", fontface = "bold") + #label was "row.names(fungEnCoordCat)" and order the same still
#   geom_text(data = fungEnCoordCont, aes(x = NMDS1, y = NMDS2), colour = "grey20", 
#             fontface = "bold", label = c("wind speed", "canopy cover", "ground veg. cover")) + #label was row.names(fungEnCoordCont) and order the same still
...
#   labs(x = "NMDS1", color = "habitat", y = "NMDS2", shape = "habitat")  + 
...
# 
# ################################
# # FINALLY, HOW ARE DISPERSIONS AFFECTED BY WIND SPEED, ETC.?
# ###############################
# 
# 
# 
...
# 
# 
# # NO LONGER USING THIS SO GRAYED OUT FOR NOW
# ##########
# 
# # # 1. First, do EUs differ in their dispersions?
# # # Calculate dispersions for each day (i.e. get average distance to centroid in multivariate space)
# # dispsByEU <- betadisper(airITSr_BCdist, group= factor(as.data.frame(as.matrix(sample_data(airITSr_noControlsTrimmed.ps)))$EU), 
# #                         type = "median")
# # # set seed!!
# # set.seed(93)
# # anova(dispsByEU, permutations = 9999)
# # # This shows that there are NO DIFFERENCES in dispersion among EUs
# # # USE NEXT LINE ONLY IF ANSWER CHANGES WITH CODE UPDATE
# # # dispsByEU.HSD <- TukeyHSD(dispsByEU) 
# # 
# # # 2. Are forest versus savanna different in their dispersion?
# # dispsByHabitat <- betadisper(airITSr_BCdist, group= factor(as.data.frame(as.matrix(sample_data(airITSr_noControlsTrimmed.ps)))$Habitat), 
# #                              type = "median")
# # # set seed!!
# # set.seed(93)
# # anova(dispsByHabitat, permutations = 9999) #not sig different (Pr (>F) = 0.5336)
# # 
# # # # # Calculate multivariate dispersions
# # # # # Calculate dispersions for each day (i.e. get average distance to centroid in multivariate space)
# # # # dispsByDate <- betadisper(airITSr_BCdist, group= factor(as.data.frame(as.matrix(sample_data(airITSr_noControlsTrimmed.ps)))$daysOut), 
# # # #                           type = "median")
# # # # 
# # # # set.seed(93)
# # # # anova(dispsByDate, permutations = 9999)
# # # # This shows that there are differences in dispersion among EUs!
# # # dispsByDate.HSD <- TukeyHSD(dispsByDate) #
# # # dispsByDate.HSD[which(dispsByDate.HSD$group[,4] < 0.05)]
# # 
# # # Pull out the (non-repeating) combinations of average speed, EU, and sampling day from wind data
# # avgWindByDay <- windData %>% tidyr::expand(nesting(avgSpeedSampPeriod, EU, actualDaysOrder))
# # 
# # # Make distances into a dataframe
# # dayDisper_df <- as.data.frame(dispsByDate$group.distances)
# # dayDisper_df <- dayDisper_df %>% rownames_to_column()
# # colnames(dayDisper_df) <- c("actualDaysOrder", "disperDay")
# # dayDisper_df$actualDaysOrder <- as.numeric(dayDisper_df$actualDaysOrder)
# # 
# # # Merge these two dataframes by actualDaysOrder
# # windDay_df <- merge(avgWindByDay, dayDisper_df, by="actualDaysOrder")
# # 
# # # Plot to see what relationship looks like: COOL! It's POSITIVE!!!
# # windByDatePlot <- ggplot(windDay_df, aes(x=avgSpeedSampPeriod, y=disperDay)) +
# #   geom_point(aes(color=EU),size=3.5) +
# #   geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) + 
# #   scale_x_continuous(name= "average wind speed (kph)", breaks= c(0, 0.25, 0.5, 0.75, 1.0, 1.25), labels = c(0, 0.25, 0.5, 0.75, 1.0, 1.25)) +
# #   scale_y_continuous(name="dispersion", breaks= c(0.15, 0.175, 0.20, 0.225, 0.25, 0.275), labels= c(0.15, 0.175, 0.20, 0.225, 0.25, 0.275)) +
# #   theme_bw() + ggtitle("Relationship between dispersion and wind speed") +
# #   #labs(title="Relationship between Dispersion and Wind Speed",x="average wind speed (kph)", y = "dispersion") +
# #   theme(legend.text = element_text(colour="black", size = 12))  + theme(legend.title = element_blank()) +
# #   theme(axis.text.x=element_text(size=14)) + theme(axis.text.y =element_text(size=14)) + theme(axis.title = element_text(size=14)) +
# #   theme(plot.title=element_text(size=16)) + scale_color_manual(values = colsForEUs)
...
# # windByDatePlot
# # 
# # cor.test(windDay_df$disperDay, windDay_df$avgSpeedSampPeriod)
# # lmDisperWind1 <- lm(disperDay ~ avgSpeedSampPeriod, data = windDay_df)
# # summary(lmDisperWind1)
# 
# ##########